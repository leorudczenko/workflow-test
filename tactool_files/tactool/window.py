...
class Window(QMainWindow):
    ...
    def setup_ui_elements(self) -> None:
        ...
        recoordination_label = QLabel("Recoordination Step Size:")
        self.recoordination_step = QSpinBox()
        self.recoordination_step.setValue(self.default_settings["recoordination_step"])
        self.recoordination_step.setMaximum(1000)
        ...
        # Recoordination widgets
        sidebar.addWidget(recoordination_label)
        sidebar.addWidget(self.recoordination_step)
    ...
    def connect_signals_and_slots(self) -> None:
        ...
        self.graphics_view.recoordinate_up.connect(lambda: self.recoordinate_analysis_points("up"))
        self.graphics_view.recoordinate_down.connect(lambda: self.recoordinate_analysis_points("down"))
        self.graphics_view.recoordinate_left.connect(lambda: self.recoordinate_analysis_points("left"))
        self.graphics_view.recoordinate_right.connect(lambda: self.recoordinate_analysis_points("right"))
    ...
    def recoordinate_analysis_points(self, key: str) -> None:
        """
        Recoordinate the existing analysis points in a given direction.
        """
        # The step value uses the recoordination input box value
        step_value = self.recoordination_step.value()
        def up(ap: AnalysisPoint) -> AnalysisPoint:
            ap.y -= step_value
            return ap
        def down(ap: AnalysisPoint) -> AnalysisPoint:
            ap.y += step_value
            return ap
        def left(ap: AnalysisPoint) -> AnalysisPoint:
            ap.x -= step_value
            return ap
        def right(ap: AnalysisPoint) -> AnalysisPoint:
            ap.x += step_value
            return ap
        directions = {
            "up": up,
        ...
        for analysis_point in current_analysis_points:
            analysis_point = directions[key](analysis_point)
